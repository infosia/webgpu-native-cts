...
| Area | Ported* | Note |
|------|--------:|------|
| `api/validation` | 70 / 129 | Y-6 V1–V4 (createBindGroup, compute_pipeline, render_pipeline fragment/vertex, render_pass descriptor/compat); 3 N/A; 56 todo |
...
|---------|------|:-----:|:----------------:|---------------|
| **yawgpu** | primary subject | ✅ green | ✅ green | **3** — F-089 (createBindGroup sampler), F-090 (fragment-state), F-092 (render-pass/attachment validation), all cross-HAL; 64 earlier findings fixed |
...
### Findings — 92 surfaced to date (F-001…F-092)
...
|--------|--:|-------------------------|
| yawgpu — open | 3 | F-089 (createBindGroup sampler), F-090 (fragment-state), F-092 (render-pass/attachment validation) — all cross-HAL, Dawn-oracle-confirmed |
