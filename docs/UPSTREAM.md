...
| yawgpu | [github.com/infosia/yawgpu](https://github.com/infosia/yawgpu) | `e56f30a5db13d03f4ec4268b426e0c090de579b4` (2026-06-02) | MIT / Apache-2.0 | **primary conformance subject**; Metal/Vulkan; vendor header `yawgpu.h`. At this revision yawgpu passes all ported `api,validation` (`pass=4332 skip=383 fail=0 crash=0`) and most `api,operation` (`command_buffer,{clearBuffer,copyBufferToBuffer}` Dawn-equal; `queue,writeBuffer` 17/17; `onSubmittedWorkDone` 5/5), with **one open finding** — **F-024** (`rgba8uint` `copyBufferToTexture`→`copyTextureToBuffer` roundtrip reads back zeros; `command_buffer,basic:{b2t2b,b2t2t2b}` triaged in `expectations/yawgpu.txt`). It additionally runs the 8 `immediate_data_size` cases Dawn skips (yawgpu `maxImmediateSize=64`). yawgpu findings F-005/006/008/009/010/011/014/016/018/020/022/023 fixed; F-015 (view-usage subset) is wgpu-native-only. See [FINDINGS](FINDINGS.md) |
