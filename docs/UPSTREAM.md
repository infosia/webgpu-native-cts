...
| yawgpu | [github.com/infosia/yawgpu](https://github.com/infosia/yawgpu) | `798fc6acfbebd763a3acc798ee587f75c4ce0df5` (2026-06-01) | MIT / Apache-2.0 | **primary conformance subject**; Metal/Vulkan; vendor header `yawgpu.h`. At this revision yawgpu **passes every ported `api,validation` test** (full suite real-GPU Metal: `pass=4332 skip=383 fail=0 crash=0 xfail=0 xpass=0`; `expectations/yawgpu.txt` has no expected failures). It passes every test it runs **and** additionally runs the 8 `immediate_data_size` cases Dawn skips (yawgpu `maxImmediateSize=64`). All yawgpu findings F-005/006/008/009/010/011/014/016/018/020/022 fixed; F-015 (view-usage subset) is wgpu-native-only. See [FINDINGS](FINDINGS.md) |
