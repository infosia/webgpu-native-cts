...
| yawgpu | [github.com/infosia/yawgpu](https://github.com/infosia/yawgpu) | `3a30443` (2026-07-02 Metal re-sweep; Vulkan last swept `05bf865` 2026-06-28, fixes `bd21cfb`) | MIT / Apache-2.0 | **primary conformance subject**; Metal and Vulkan HALs. **WGSL frontend = Tint** (Dawn's shader compiler), so shader compile/validation is Dawn-equivalent. Runs the ported suite **crash-free** with `shader/execution` `fail=0` on native Metal _and_ native Vulkan (NVIDIA RTX 5060 Ti). Metal sweep 2026-07-02 (`3a30443`): pass 1,990,512 / skip 105,405 — `api/validation` 292,960, `api/operation` 228,595, `shader/execution` 822,209, `shader/validation` 646,748 (per-subcase). A recent update (real hardware-limit reporting + `subgroups` exposure) raised Metal coverage to Dawn's level but surfaced **19 open Metal defects** Dawn does not have: **F-142** (5 `requestDevice` real-limits inconsistencies) + **F-143** (14 subgroup validation gaps). Expected failures: the spec-in-flux Vulkan cases in `expectations/yawgpu-vulkan.txt` (sample_mask/position semantics, MoltenVK-only artifacts) and the 2 shared `index_buffer_format_dirtying` port-oracle non-defects. Vendor header `yawgpu.h` exists but its extension surface is **not yet exercised** by the suite. See [FINDINGS](FINDINGS.md). |
