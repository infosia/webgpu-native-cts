...
| yawgpu | [github.com/infosia/yawgpu](https://github.com/infosia/yawgpu) | `e56f30a5db13d03f4ec4268b426e0c090de579b4` (2026-06-02) | MIT / Apache-2.0 | **primary conformance subject**; Metal/Vulkan; vendor header `yawgpu.h`. At this revision yawgpu **passes every ported test — both `api,validation` and `api,operation`** (real-GPU Metal: `api,validation` `pass=4332 skip=383 fail=0 crash=0`; `api,operation,command_buffer` `pass=5 fail=0 crash=0`, Dawn-equal; `expectations/yawgpu.txt` has no expected failures). It passes every test it runs **and** additionally runs the 8 `immediate_data_size` cases Dawn skips (yawgpu `maxImmediateSize=64`). All yawgpu findings F-005/006/008/009/010/011/014/016/018/020/022/023 fixed; F-015 (view-usage subset) is wgpu-native-only. See [FINDINGS](FINDINGS.md) |
