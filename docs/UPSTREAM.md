...
| yawgpu | [github.com/infosia/yawgpu](https://github.com/infosia/yawgpu) | `c893eaca619cfc4c7f74310fd55c44a7c313fd6f` (2026-06-02) | MIT / Apache-2.0 | **primary conformance subject**; Metal/Vulkan; vendor header `yawgpu.h`. At this revision yawgpu **passes every ported test through T23** (real-GPU Metal: `api,validation` `pass=4332 skip=383 fail=0 crash=0`; `api,operation` — `command_buffer,{clearBuffer,copyBufferToBuffer,basic}` Dawn-equal, `queue,writeBuffer` 17/17, `onSubmittedWorkDone` 5/5). The new color `image_copy` port (T24b) surfaces two **open** findings — **F-025** (`queueWriteTexture` writes zeros) and **F-026** (`copyBufferToTexture`/`copyTextureToBuffer` mishandle non-default buffer layout + mip levels) — so the full `image_copy` run is `pass=21860 fail=115396` pending the yawgpu fix; `expectations/yawgpu.txt` still has **no** expected failures (findings surfaced, not masked). It additionally runs the 8 `immediate_data_size` cases Dawn skips (yawgpu `maxImmediateSize=64`). Earlier yawgpu findings F-005/006/008/009/010/011/014/016/018/020/022/023/024 are fixed; F-015 (view-usage subset) is wgpu-native-only. See [FINDINGS](FINDINGS.md) |
