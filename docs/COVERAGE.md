...
| `api/validation/createTexture.spec.ts` | partial | 10 ported (`sample_count,1d_2d_array_3d`, `dimension_type_and_format_compatibility`, `sampleCount,various_sampleCount_with_all_formats`, `zero_size_and_usage`, `mipLevelCount,format`, `mipLevelCount,bound_check`, `mipLevelCount,bound_check,bigger_than_integer_bit_width`, `texture_size,1d_texture`, `texture_size,2d_texture,uncompressed_format`, `texture_size,3d_texture,uncompressed_format`) | `texture_size,{2d,3d}_texture,compressed_format` (block-scaled variants + max-valid-size helper → T4b); `texture_size,default_value_and_smallest_size,*` **N/A** (JS array-length defaulting `[1]`/`[1,1]`/`[1,1,1]` collapses to `{1,1,1}` in C); `texture_usage`/`usage`/`new_usages`/`viewFormats*` (need renderable+usage capability tables → T5); createView | **Texture file T1–T4.** On `AllFeaturesMaxLimitsGpuTest`. `texture_format.h` holds all **101** formats (49 uncompressed + 52 compressed BC/ETC2/ASTC) + `kRegularTextureFormats` (43) + `multisample`/class/predicates + tier1-blendable set + `maxMipLevelCount` + `textureFormatAndDimensionPossiblyCompatible`; helpers `createTextureTracked`, `skipIfTextureFormatNotSupported`, `textureDimensionAndFormatCompatibleForDevice` (+3D BC/ASTC sliced exception), `skipIfTextureFormatAndDimensionNotCompatible`, `isTextureFormatMultisampled`. Counts: `dimension_type…` 404, `sampleCount,various…` 202/8, `zero_size` 13/78, `mipLevelCount,format` 330/4520, `mipLevelCount,bound_check` 2/21, `texture_size,1d` 43/12, `texture_size,2d,uncompressed` 882, `texture_size,3d,uncompressed` 43/9 — all green on **wgpu-native & Dawn**. **yawgpu fails/crashes** on the 12 rejected-as-`Undefined` color formats + `Depth24PlusStencil8` ([F-005](FINDINGS.md)) and 6 multisample-capability formats ([F-006](FINDINGS.md)); triaged in `expectations/yawgpu.txt`. T3 drove the **per-case subcase-expansion** harness fix; size-variant arrays are encoded as integer indexes (array `Value` unsupported). `widthVariant`/`sizeVariant` deviation noted in the file header |
