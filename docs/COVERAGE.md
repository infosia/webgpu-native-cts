...
| `api/validation/createView.spec.ts` | partial | 5 / 10 (`aspect`, `format`, `dimension`, `cube_faces_square`, `texture_state`) | not yet ported: `array_layers`, `mip_levels` (need `reifyTextureViewDescriptor` + layer/level range validation), `texture_view_usage{,_of_multiple_usages,_with_view_format}` (need a view-usage table) | **createView file T8–T9.** On `AllFeaturesMaxLimitsGpuTest`. `createViewTracked` + `kTextureAspects`(3) + `isDepth/isStencilTextureFormat` (T8); `kTextureViewDimensions`(6) + `getTextureDimensionFromView` + `createTextureWithState` (T9). T8 `aspect` 303 (101×3) + `format` 36/21614 (=101×107×2, reuses T7 view-compat/feature/`expand`) — green on all three. T9 `dimension` 21, `cube_faces_square` 15, `texture_state` 1/3 — **Dawn passes all 37** (reference). **yawgpu fails 12** ([F-011](FINDINGS.md): rejects 2D-view-of-multilayer + Cube views, doesn't enforce CubeArray square); **wgpu-native fails 1** ([F-012](FINDINGS.md): rejects createView on a destroyed texture). Triaged in `expectations/{yawgpu,wgpu-native}.txt` |
