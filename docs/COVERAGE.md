...
| `api/validation/createTexture.spec.ts` | partial | 12 ported (`sample_count,1d_2d_array_3d`, `dimension_type_and_format_compatibility`, `sampleCount,various_sampleCount_with_all_formats`, `zero_size_and_usage`, `mipLevelCount,format`, `mipLevelCount,bound_check`, `mipLevelCount,bound_check,bigger_than_integer_bit_width`, `texture_size,1d_texture`, `texture_size,{2d,3d}_texture,uncompressed_format`, `texture_size,{2d,3d}_texture,compressed_format`) | `texture_size,default_value_and_smallest_size,*` **N/A** (JS array-length defaulting `[1]`/`[1,1]`/`[1,1,1]` collapses to `{1,1,1}` in C); `texture_usage`/`usage`/`new_usages`/`viewFormats*` (need renderable+usage capability tables → T5); createView | **Texture file T1–T4b.** On `AllFeaturesMaxLimitsGpuTest`. `texture_format.h` holds all **101** formats (49 uncompressed + 52 compressed BC/ETC2/ASTC) + `kRegularTextureFormats` (43) + `multisample`/class/predicates + tier1-blendable set + `maxMipLevelCount` + `textureFormatAndDimensionPossiblyCompatible` + `roundDown`/`getMaxValidTextureSizeForFormatAndDimension` + shared 28-entry compressed size-variant table; helpers `createTextureTracked`, `skipIfTextureFormatNotSupported`, `textureDimensionAndFormatCompatibleForDevice` (+3D BC/ASTC sliced exception), `skipIfTextureFormatAndDimensionNotCompatible`, `isTextureFormatMultisampled`. Counts incl. `texture_size,2d,uncompressed` 882, `…,2d,compressed` 104/28, `…,3d,compressed` 52/28 — all green on **wgpu-native & Dawn** (both run all compression families; ETC2/EAC 3D skip as dimension-incompatible). **yawgpu fails/crashes** on the 12 rejected-as-`Undefined` color formats + `Depth24PlusStencil8` ([F-005](FINDINGS.md)) and 6 multisample-capability formats ([F-006](FINDINGS.md)); **yawgpu skips all compressed** (no BC/ETC2/ASTC adapter feature here). T3 drove the **per-case subcase-expansion** harness fix; size-variant arrays are encoded as integer indexes (array `Value` unsupported, noted in the file header) |
