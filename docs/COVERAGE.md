...
| `api/validation/createView.spec.ts` | partial | 2 / 10 (`aspect`, `format`) | not yet ported: `dimension`, `array_layers`, `mip_levels`, `cube_faces_square` (need `reifyTextureViewDescriptor` + `kTextureViewDimensions`), `texture_state`, `texture_view_usage{,_of_multiple_usages,_with_view_format}` (need a view-usage table) | **First createView file (T8).** On `AllFeaturesMaxLimitsGpuTest`. New `createViewTracked` (+ `WGPUTextureView` release tracking), `kTextureAspects` (3), `isDepthTextureFormat`/`isStencilTextureFormat` (= `hasDepth`/`hasStencil`). `aspect` 303 cases (101×3); `format` 36 cases (feature pairs) × 21614 subcases (=101×107×2) reusing T7's `textureFormatsAreViewCompatible` / `kFeaturesForFormats` / `filterFormatsByFeature` / `ParamsBuilder::expand`. **Green on all three — Dawn = yawgpu (full pass, no divergence)**; wgpu-native passes, skipping only the tier1-feature cases its adapter lacks (18 aspect + 6 format). No findings |
