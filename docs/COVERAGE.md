...
| `api/validation/createTexture.spec.ts` | partial | 3 ported (`sample_count,1d_2d_array_3d`, `dimension_type_and_format_compatibility` over **all 101** formats, `sampleCount,various_sampleCount_with_all_formats`) | the other ~18 createTexture tests + `texture_size,*` (JS array-length defaulting) + `viewFormats*` + createView deferred to T3 | **First texture file (T1+T2).** Now on `AllFeaturesMaxLimitsGpuTest` (all adapter features + max limits). `texture_format.h` holds all **101** formats (49 uncompressed + 52 compressed BC/ETC2/ASTC, count parity vs upstream) + `multisample`/class + `isBC/isASTC/isETC2/isCompressed` + tier1-blendable set; helpers `createTextureTracked`, `skipIfTextureFormatNotSupported`, `textureDimensionAndFormatCompatibleForDevice` (+3D BC/ASTC sliced exception), `isTextureFormatMultisampled`. `dimension_type…` 404 cases (4×101), `sampleCount,various…` 202 cases/8 subcases — both green on **wgpu-native & Dawn** (adapter feature exposure differs: Dawn runs the full matrix, wgpu-native skips tier1/depth32fs8, both skip nothing fatal). **yawgpu fails/crashes** on 12 rejected-as-`Undefined` color formats + `Depth24PlusStencil8` ([F-005](FINDINGS.md)) and 6 multisample-capability formats ([F-006](FINDINGS.md)); triaged in `expectations/yawgpu.txt` |
