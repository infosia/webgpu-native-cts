...
```bash
# Build wgpu-native first (in your wgpu-native checkout): `cargo build --release -j 8` (or `make`),
...
cargo build --release --features metal -j 8   # produces target/release/libyawgpu.{a,dylib}
...
>
> ```bash
> cargo build --release -p yawgpu --features metal -j 8   # also builds out/build/libtint_shim.dylib
...
> cmake --build build-yawgpu --target cts -j 8
...
CARGO_TARGET_DIR=<yawgpu>/target-vulkan \
  cargo build --release -p yawgpu --features vulkan -j 8

cmake -S . -B build-yawgpu-vulkan \
      -DCTS_BACKEND=yawgpu \
      -DCTS_YAWGPU_DIR=<yawgpu> \
      -DCTS_YAWGPU_LIB=<yawgpu>/target-vulkan/release/libyawgpu.a
cmake --build build-yawgpu-vulkan --target cts -j 8
```
...
cmake --build <dawn>/out/Release --config Release --target webgpu_dawn -j 8
...
# Build everything.
# Convention: build in parallel (-j 8). The constraint in this repo is run-time
# memory, not compilation — see CLAUDE.md for the cgroup cap on cts runs.
...
| `--workers N` \| `auto` | parallel runner: spawn `N` shard workers on one machine, merge their machine-readable results back into the same ordered text summary as a sequential run. POSIX uses `fork`+`exec`; Windows uses `CreateProcess`. Each worker loads only its own shard's slice of the parent's ordered case plan (one temp plan file per shard, entries tagged with global case positions) instead of re-enumerating cases. `N=1` is sequential; `auto` (or `0`) resolves to `min(hardware_concurrency, 8)` — capped low because GPU/VRAM pressure (not CPU) is the limiter and high counts can wedge the OS. **Composes with `--isolate`** (parallel per-case isolation pool — see that row); still incompatible with `--crash-list`. Does **not** compose with `--shard` in the plain parallel runner: that is a hard startup error. Shard a parallel run by batching queries, or use `--isolate --workers` for a sharded per-case pool |
| `--shard I/N` | run or list only the deterministic round-robin shard where case index `idx % N == I`; works with `--list-cases` for partition checks and can be used directly in CI on any platform. Does **not** compose with plain `--workers` (hard startup error); shard a parallel run by batching queries, or use `--isolate --workers` |
...
| `--shard-results` / `--shard-from K` / `--case-plan <file>` | (internal, the `--workers` worker protocol) a spawned shard worker emits machine-readable `RESULT` lines (`--shard-results`), resumes after a crash from case index `K` (`--shard-from`), and loads its shard's slice of the parent's ordered case plan instead of re-enumerating (`--case-plan`; `K` is a global case position). Never pass these by hand |
...
```bash
cmake --build build --target gen_listings -j 8
