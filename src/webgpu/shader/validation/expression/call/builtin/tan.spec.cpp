...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/tan.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,tan", "Validation tests for the tan() builtin.");

// AbstractInt is converted to AbstractFloat before calling into the builtin.
static bt::ScalarKind floatResultKind(const bt::Type& ty) {
    const bt::ScalarKind k = bt::scalarTypeOf(ty).kind;
    return k == bt::ScalarKind::AbstractInt ? bt::ScalarKind::AbstractFloat : k;
}

// fp.constants().positive.min for the float result kind.
static double smallestPositive(bt::ScalarKind k) {
    switch (k) {
        case bt::ScalarKind::F32:
            return b::kValueF32().posMin;
        case bt::ScalarKind::F16:
            return b::kValueF16().posMin;
        default:  // abstract-float -> f64 smallest positive normal
            return b::reinterpretU64AsF64(b::KBit::f64_pos_min);
    }
}

// fp.quantize for the float result kind.
static double quantizeForKind(bt::ScalarKind k, double v) {
    switch (k) {
        case bt::ScalarKind::F32:
            return cts::shader_validation::binary::quantizeToF32(v);
        case bt::ScalarKind::F16:
            return cts::shader_validation::binary::quantizeToF16(v);
        default:
            return v;
    }
}
...
CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of tan() rejects invalid "
          "values")
        ...
        return u.combine("stage", b::kConstantAndOverrideStages())
            ...
            .expand("value", [](const ParamRecord& p) {
                ...
                return b::rangeValues(b::uniqueRanges(
                    {b::minusThreePiToThreePiRangeForType(ty), b::fullRangeForType(ty)}));
            });
    })
    .fn([](ShaderValidationTest& t) {
        ...
        const bt::ScalarKind k = floatResultKind(ty);
        const double v = quantizeForKind(k, num);
        const bool expectedResult = std::abs(std::cos(v)) > smallestPositive(k);
        b::validateConstOrOverrideBuiltinEval(t, "tan", expectedResult,
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of tan with variously shaped and typed arguments")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(std::string(c.name) == "good",
                              std::string("const c = tan") + c.suffix + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of tan must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, "fn f() { " + useIt + "tan(1.2); }");
    });
