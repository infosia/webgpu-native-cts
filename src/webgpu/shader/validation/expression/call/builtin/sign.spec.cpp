...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/sign.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,sign", "Validation tests for the sign() builtin.");

// kValuesTypes = kFloatScalarsAndVectors ++ kConcreteSignedIntegerScalarsAndVectors
static std::vector<bt::Type> kValuesTypes() {
    std::vector<bt::Type> out = b::kFloatScalarsAndVectors();
    for (const bt::Type& t : b::kConcreteSignedIntegerScalarsAndVectors()) {
        out.push_back(t);
    }
    return out;
}
...
CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of sign() inputs rejects "
          "invalid values")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        b::validateConstOrOverrideBuiltinEval(t, "sign", /*expectedResult=*/true,
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of sign with variously shaped and typed arguments")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(std::string(c.name) == "good",
                              std::string("const c = sign") + c.suffix + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of sign must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, "fn f() { " + useIt + "sign(1.0); }");
    });
