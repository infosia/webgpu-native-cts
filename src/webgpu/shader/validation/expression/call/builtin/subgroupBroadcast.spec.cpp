...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/subgroupBroadcast.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,subgroupBroadcast",
...
CTS_TEST(g, "must_use")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string wgsl = std::string("\nenable subgroups;\n@compute @workgroup_size(16)\n"
                                             "fn main() {\n  ") +
                                 (mustUse ? "_ = " : "") + "subgroupBroadcast(0, 0);\n}";
...
CTS_TEST(g, "return_type")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string wgsl = std::string("\n") + enables +
                                 "\n@compute @workgroup_size(1)\nfn main() {\n  let res : " +
                                 retType.toString() + " = subgroupBroadcast(" +
...
CTS_TEST(g, "id_values")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string wgsl = std::string("\nenable subgroups;\n\nconst c = ") +
                                 std::to_string(value) + ";\n\nfn foo() {\n  _ = subgroupBroadcast(0, " +
                                 arg + ");\n}";
        const bool expect = value >= 0 && value < 128;
        t.expectCompileResult(expect, wgsl);
    });
