...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/textureSample.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// Upstream CTS: Copyright (c) 2022 The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) 2026 Kota Iguchi, BSD-3-Clause.
//
// Validation tests for the textureSample() builtin: coords / array_index /
// offset parameter types, the const-expression and [-8,7] range requirements on
// offset, the fragment-only stage restriction, the return type, and incompatible
// texture types. The kValidTextureSampleParameterTypes table is ported locally
// (returnType / coordsArgType / hasArrayIndexArg / offsetArgType per textureType),
// preserving the upstream object key order.
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,textureSample",
    "Validation tests for the textureSample() builtin.");

// kValidTextureSampleParameterTypes (object key order preserved).
struct SampleArgs {
...
const std::vector<SampleArgs>& kValidTextureSampleParameterTypes() {
    using bt::ScalarKind;
    static const bt::Type vec4f = bt::vec(4, ScalarKind::F32);
    static const bt::Type f32 = bt::scalar(ScalarKind::F32);
    static const bt::Type none{};  // placeholder when hasOffset == false
    static const std::vector<SampleArgs> v = {
        {"texture_1d<f32>", vec4f, f32, false, none, false},
        {"texture_2d<f32>", vec4f, bt::vec(2, ScalarKind::F32), false, bt::vec(2, ScalarKind::I32),
         true},
        {"texture_2d_array<f32>", vec4f, bt::vec(2, ScalarKind::F32), true,
         bt::vec(2, ScalarKind::I32), true},
        {"texture_3d<f32>", vec4f, bt::vec(3, ScalarKind::F32), false, bt::vec(3, ScalarKind::I32),
         true},
        {"texture_cube<f32>", vec4f, bt::vec(3, ScalarKind::F32), false, none, false},
        {"texture_cube_array<f32>", vec4f, bt::vec(3, ScalarKind::F32), true, none, false},
        {"texture_depth_2d", f32, bt::vec(2, ScalarKind::F32), false, bt::vec(2, ScalarKind::I32),
         true},
        {"texture_depth_2d_array", f32, bt::vec(2, ScalarKind::F32), true,
         bt::vec(2, ScalarKind::I32), true},
        {"texture_depth_cube", f32, bt::vec(3, ScalarKind::F32), false, none, false},
        {"texture_depth_cube_array", f32, bt::vec(3, ScalarKind::F32), true, none, false},
    };
    return v;
}
const SampleArgs* sampleArgsByName(const std::string& name) {
    for (const SampleArgs& s : kValidTextureSampleParameterTypes()) {
...
CTS_TEST(g, "return_type")
    .desc("Validates the return type of textureSample is the expected type.")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            ...
            " = textureSample(t, s, " + coordWGSL + arrayWGSL + offsetWGSL +
            ");\n  return vec4f(0);\n}\n";
        const bool expectSuccess = bt::isConvertible(s->returnType, returnVarType);
        t.expectCompileResult(expectSuccess, code);
    });

CTS_TEST(g, "coords_argument")
    .desc("Validates that only incorrect coords arguments are rejected by textureSample")
    .params([](ParamsBuilder u) {
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            "\n@group(0) @binding(0) var s: sampler;\n@group(0) @binding(1) var t: " + textureType +
            ";\n@fragment fn fs() -> @location(0) vec4f {\n  let v = textureSample(t, s, " +
            coordWGSL + arrayWGSL + offsetWGSL + ");\n  return vec4f(0);\n}\n";
        const bool expectSuccess = bt::isConvertible(coordArgType, s->coordsArgType);
        t.expectCompileResult(expectSuccess, code);
    });

CTS_TEST(g, "array_index_argument")
    .desc("Validates that only incorrect array_index arguments are rejected by textureSample")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
            .filter([](const ParamRecord& p) {
                const SampleArgs* s =
...
CTS_TEST(g, "offset_argument")
    .desc("Validates that only incorrect offset arguments are rejected by textureSample")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
            .filter([](const ParamRecord& p) {
                const SampleArgs* s =
...
CTS_TEST(g, "offset_argument,non_const")
    .desc("Validates that only non-const offset arguments are rejected by textureSample")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            ...
            ";\n@fragment fn fs() -> @location(0) vec4f {\n  const c = 1;\n  let l = " +
            bt::createWgsl(s->offsetArgType, 0) + ";\n  let v = textureSample(t, s, " + coordWGSL +
            arrayWGSL + ", " + offsetWGSL + ");\n  return vec4f(0);\n}\n";
        const bool expectSuccess = varType == "c";
        t.expectCompileResult(expectSuccess, code);
    });

CTS_TEST(g, "only_in_fragment")
    .desc("Validates that textureSample must not be used in a compute or vertex shader.")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            ...
            "t: " +
            textureType + ";\n\nfn foo() {\n  _ = textureSample(t, s, " + coordWGSL + arrayWGSL +
            offsetWGSL + ");\n}";
        t.expectCompileResult(config.expectSuccess, code);
    });

CTS_TEST(g, "texture_type")
    .desc("Validates that incompatible texture types don't work with textureSample")
    .params([](ParamsBuilder u) {
...
CTS_TEST(g, "must_use")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            ...
            "sampler;\n    fn foo() {\n      " +
            std::string(use ? "_ =" : "") + " textureSample(t, s, vec2(0,0));\n    }";
