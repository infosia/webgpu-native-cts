...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/mix.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// Upstream CTS: Copyright (c) 2022 The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) 2026 Kota Iguchi, BSD-3-Clause.
//
// kValidArgumentTypes = kConvertableToFloatVectors. `values` validates both the
// all-vector form mix(vecN(a),vecN(b),vecN(c)) and the scalar-blend form
// mix(vecN(a),vecN(b),c). ConstantOrOverrideValueChecker tracks the intermediate
// mix equation a*(1-c)+b*c for OOB.
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,mix", "Validation tests for the mix() builtin.");
...
CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of mix() never errors")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const double c1 = vCheck.checkedResult(1 - c);
        const double ac1 = vCheck.checkedResult(a * c1);
        const double bc = vCheck.checkedResult(bb * c);
        vCheck.checkedResult(ac1 + bc);

        b::validateConstOrOverrideBuiltinEval(
            t, "mix", vCheck.allChecksPassed(),
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of mix with variously shaped and typed arguments")
    .params([](ParamsBuilder u) { return u.combine("arg", argNames()); })
    .fn([](ShaderValidationTest& t) {
        const std::string arg = t.param<std::string>("arg");
        t.expectCompileResult(arg == "good", std::string("const c = mix") + argSuffix(arg) + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of mix must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, std::string("fn f() { ") + useIt + "mix" + argSuffix("good") +
                                       "; }");
    });
