...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/degrees.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,degrees",
    "Validation tests for the degrees() builtin.");
...
CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of degrees() inputs rejects "
          "invalid values")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const bool expectedResult = b::isRepresentable((num * 180.0) / PI, floatResultKind(ty));
        b::validateConstOrOverrideBuiltinEval(t, "degrees", expectedResult,
                                              {b::createBuiltinValue(ty, rv)}, stage);
    });

CTS_TEST(g, "integer_argument")
    .desc("Validates that scalar and vector integer arguments are rejected by degrees()")
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of degrees with variously shaped and typed arguments")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(std::string(c.name) == "good",
                              std::string("const c = degrees") + c.suffix + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of degrees must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, "fn f() { " + useIt + "degrees(1.1); }");
    });
