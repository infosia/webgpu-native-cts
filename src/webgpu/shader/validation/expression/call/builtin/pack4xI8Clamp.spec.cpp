...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/pack4xI8Clamp.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
constexpr const char* kFn = "pack4xI8Clamp";

TestGroup<ShaderValidationTest> g =
    MakeTestGroup<ShaderValidationTest>("shader,validation,expression,call,builtin,pack4xI8Clamp",
                                        "Validate pack4xI8Clamp");

// kArgCases (object order preserved). `good` == "(vec4i())".
struct ArgCase {
    const char* name;
    const char* args;
};
...
static const char* kGoodArgs = "(vec4i())";
...
CTS_TEST(g, "unsupported")
    .desc("Test absence of pack4xI8Clamp when packed_4x8_integer_dot_product is not supported.")
...
CTS_TEST(g, "supported")
    .desc("Test presence of pack4xI8Clamp when packed_4x8_integer_dot_product is supported.")
    .params([](ParamsBuilder u) { return u.combine("requires", {Value(false), Value(true)}); })
    .fn([](ShaderValidationTest& t) {
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of pack4xI8Clamp with various numbers of and types of arguments")
...
CTS_TEST(g, "must_use")
    .desc("Result of pack4xI8Clamp must be used")
