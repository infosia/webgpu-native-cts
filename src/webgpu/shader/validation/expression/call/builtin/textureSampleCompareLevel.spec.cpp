...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/textureSampleCompareLevel.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// Upstream CTS: Copyright (c) 2022 The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) 2026 Kota Iguchi, BSD-3-Clause.
//
// Validation tests for the textureSampleCompareLevel() builtin: coords /
// array_index / depth_ref / offset parameter types, the const-expression and
// [-8,7] range requirements on offset, the f32 return type, and incompatible
// texture types. Only the four depth texture types are valid; the sampler is a
// sampler_comparison. The kValidTextureSampleCompareLevelParameterTypes table is
// ported locally (coordsArgType / hasArrayIndexArg / offsetArgType per
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    ...
    "Validation tests for the textureSampleCompareLevel() builtin.");

// kValidTextureSampleCompareLevelParameterTypes (object key order preserved).
struct SampleArgs {
...
const std::vector<SampleArgs>& kValidTextureSampleCompareLevelParameterTypes() {
    using bt::ScalarKind;
...
const SampleArgs* sampleArgsByName(const std::string& name) {
    for (const SampleArgs& s : kValidTextureSampleCompareLevelParameterTypes()) {
...
CTS_TEST(g, "return_type")
    .desc("Validates the return type of textureSampleCompareLevel is the expected type.")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            ...
            " = textureSampleCompareLevel(t, s, " + coordWGSL + arrayWGSL + ", 0" + offsetWGSL +
            ");\n  return vec4f(0);\n}\n";
...
CTS_TEST(g, "coords_argument")
    .desc("Validates that only incorrect coords arguments are rejected by textureSampleCompareLevel")
    .params([](ParamsBuilder u) {
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            "\n@group(0) @binding(0) var s: sampler_comparison;\n@group(0) @binding(1) var t: " +
            textureType +
            ";\n@fragment fn fs() -> @location(0) vec4f {\n  let v = textureSampleCompareLevel(t, "
            "s, " +
...
CTS_TEST(g, "depth_ref_argument")
    .desc("Validates that only incorrect depth_ref arguments are rejected by "
          "textureSampleCompareLevel")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
...
CTS_TEST(g, "offset_argument")
    .desc("Validates that only incorrect offset arguments are rejected by textureSampleCompareLevel")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
            .filter([](const ParamRecord& p) {
                const SampleArgs* s =
...
CTS_TEST(g, "offset_argument,non_const")
    .desc("Validates that only non-const offset arguments are rejected by textureSampleCompareLevel")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            ...
            ";\n@fragment fn fs() -> @location(0) vec4f {\n  const c = 1;\n  let l = " +
            bt::createWgsl(s->offsetArgType, 0) + ";\n  let v = textureSampleCompareLevel(t, s, " +
            coordWGSL + arrayWGSL + ", 0, " + offsetWGSL + ");\n  return vec4f(0);\n}\n";
...
CTS_TEST(g, "texture_type")
    .desc("Validates that incompatible texture types don't work with textureSampleCompareLevel")
    .params([](ParamsBuilder u) {
...
CTS_TEST(g, "must_use")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            ...
            ": sampler_comparison;\n    fn foo() {\n      " +
            std::string(use ? "_ =" : "") +
            " textureSampleCompareLevel(t, s, vec2(0,0), 0);\n    }";
