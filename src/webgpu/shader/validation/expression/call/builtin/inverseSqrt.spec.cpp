...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/inverseSqrt.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,inverseSqrt",
    "Validation tests for the inverseSqrt() builtin.");
...
CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of inverseSqrt() inputs "
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const bool expectedResult =
            num > 0 && b::isRepresentable(1.0 / std::sqrt(num), floatResultKind(ty));
        b::validateConstOrOverrideBuiltinEval(t, "inverseSqrt", expectedResult,
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of inverseSqrt with variously shaped and typed arguments")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(std::string(c.name) == "good",
                              std::string("const c = inverseSqrt") + c.suffix + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of inverseSqrt must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, "fn f() { " + useIt + "inverseSqrt(1.2); }");
    });
