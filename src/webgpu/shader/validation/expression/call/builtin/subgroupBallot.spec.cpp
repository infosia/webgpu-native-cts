...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/subgroupBallot.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,subgroupBallot",
...
CTS_TEST(g, "return_type")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string wgsl = std::string("\n") + enables +
                                 "\n@compute @workgroup_size(1)\nfn main() {\n  let res : " +
                                 type.toString() + " = subgroupBallot(true);\n}";
        t.expectCompileResult(type == bt::vec(4, bt::ScalarKind::U32), wgsl);
    });
