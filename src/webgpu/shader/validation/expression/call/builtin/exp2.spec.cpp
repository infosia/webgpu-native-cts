...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/exp2.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,exp2", "Validation tests for the exp2() builtin.");
...
// rangeForType(number_range, bigint_range) used by exp2().
static std::vector<b::RangeValue> valueForType(const bt::Type& ty) {
    const std::vector<double> nums = {
        -1e2, -1e3, -4, -3, -2, -1, -1e-1, -1e-2, -1e-3, 0,
        1e-3, 1e-2, 1e-1, 1, 2, 3, 4, 1e2, 1e3,
        std::log2(b::kValueF16().posMax) - 0.1, std::log2(b::kValueF16().posMax) + 0.1,
        std::log2(b::kValueF32().posMax) - 0.1, std::log2(b::kValueF32().posMax) + 0.1,
    };
    const std::vector<int64_t> bigints = {-100, -1000, -4, -3, -2, -1, 0, 1, 2, 3, 4, 100, 1000};
    return b::rangeForType(ty, nums, bigints);
}

CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of exp2() rejects invalid "
          "values")
    .params([](ParamsBuilder u) {
        return u.combine("stage", b::kConstantAndOverrideStages())
            ...
            .expand("value", [](const ParamRecord& p) {
                const bt::Type ty = bt::typeByName(valueAs<std::string>(*findParam(p, "type")));
                return b::rangeValues(valueForType(ty));
            });
    })
    .fn([](ShaderValidationTest& t) {
        ...
        const bool expectedResult = b::isRepresentable(std::pow(2.0, num), floatResultKind(ty));
        b::validateConstOrOverrideBuiltinEval(t, "exp2", expectedResult,
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of exp2 with variously shaped and typed arguments")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(std::string(c.name) == "good",
                              std::string("const c = exp2") + c.suffix + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of exp2 must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, "fn f() { " + useIt + "exp2(1.2); }");
    });
