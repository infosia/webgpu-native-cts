...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/log.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,log", "Validation tests for the log() builtin.");

CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of log() inputs rejects "
          "invalid values")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        b::validateConstOrOverrideBuiltinEval(t, "log", expectedResult,
...
CTS_TEST(g, "integer_argument")
    .desc("Validates that scalar and vector integer arguments are rejected by log()")
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of log with variously shaped and typed arguments")
    .params([](ParamsBuilder u) { return u.combine("arg", argNames()); })
    .fn([](ShaderValidationTest& t) {
        const std::string arg = t.param<std::string>("arg");
        const std::string code = std::string("const c = log") + findArgs(arg) + ";";
        t.expectCompileResult(arg == "good", code);
    });

CTS_TEST(g, "must_use")
    .desc("Result of log must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, "fn f() { " + useIt + "log(1.1); }");
    });
