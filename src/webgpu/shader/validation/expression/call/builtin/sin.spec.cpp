...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/sin.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,sin", "Validation tests for the sin() builtin.");

CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of sin() rejects invalid "
          "values")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        b::validateConstOrOverrideBuiltinEval(t, "sin", /*expectedResult=*/true,
...
CTS_TEST(g, "integer_argument")
    .desc("Validates that scalar and vector integer arguments are rejected by sin()")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        b::validateConstOrOverrideBuiltinEval(t, "sin", expectedResult,
                                              {b::createBuiltinValue(ty, b::RangeValue::makeI(0))},
                                              "constant");
    });

// kArgCases (object key order preserved).
struct ArgCase {
    const char* name;
    const char* args;
};
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of sin with variously shaped and typed arguments")
    .params([](ParamsBuilder u) { return u.combine("arg", argNames()); })
    .fn([](ShaderValidationTest& t) {
        const std::string arg = t.param<std::string>("arg");
        const std::string code = std::string("const c = sin") + findArgs(arg) + ";";
        t.expectCompileResult(arg == "good", code);
    });

CTS_TEST(g, "must_use")
    .desc("Result of sin must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, "fn f() { " + useIt + "sin(1.1); }");
    });
