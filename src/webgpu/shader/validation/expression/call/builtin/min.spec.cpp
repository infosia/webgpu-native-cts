...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/min.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,min", "Validation tests for the min() builtin.");

CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of min() never errors")
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of min with variously shaped and typed arguments")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(arg == "good",
                              std::string("const c = min") + argSuffix(arg) + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of min must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, std::string("fn f() { ") + useIt + "min" + argSuffix("good") +
                                       "; }");
    });
