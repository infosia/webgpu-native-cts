...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/sqrt.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,sqrt", "Validation tests for the sqrt() builtin.");
...
CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of sqrt() inputs rejects "
          "invalid values")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const bool expectedResult = num >= 0 && b::isRepresentable(std::sqrt(num), floatResultKind(ty));
        b::validateConstOrOverrideBuiltinEval(t, "sqrt", expectedResult,
                                              {b::createBuiltinValue(ty, rv)}, stage);
    });

CTS_TEST(g, "integer_argument")
    .desc("Validates that scalar and vector integer arguments are rejected by sqrt()")
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of sqrt with variously shaped and typed arguments")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(std::string(c.name) == "good",
                              std::string("const c = sqrt") + c.suffix + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of sqrt must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, "fn f() { " + useIt + "sqrt(1.1); }");
    });
