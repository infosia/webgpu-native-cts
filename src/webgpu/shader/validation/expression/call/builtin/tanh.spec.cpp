...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/tanh.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,tanh", "Validation tests for the tanh() builtin.");
...
CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of tanh() rejects invalid "
          "values")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const bool expectedResult = b::isRepresentable(std::tanh(num), floatResultKind(ty));
        b::validateConstOrOverrideBuiltinEval(t, "tanh", expectedResult,
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of tanh with variously shaped and typed arguments")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(std::string(c.name) == "good",
                              std::string("const c = tanh") + c.suffix + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of tanh must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, "fn f() { " + useIt + "tanh(1.2); }");
    });
