...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/max.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,max", "Validation tests for the max() builtin.");

CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of max() never errors")
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of max with variously shaped and typed arguments")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(arg == "good",
                              std::string("const c = max") + argSuffix(arg) + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of max must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, std::string("fn f() { ") + useIt + "max" + argSuffix("good") +
                                       "; }");
    });
