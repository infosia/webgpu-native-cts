...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/extractBits.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// Upstream CTS: Copyright (c) 2022 The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) 2026 Kota Iguchi, BSD-3-Clause.
//
// kValuesTypes = kConcreteIntegerScalarsAndVectors. extractBits(e, offset, count)
// with offset/count u32 literals. The `values` test sweeps fullRangeForType and a
// fixed (offset,count) table; `count_offset` builds custom WGSL across
// constant/override/runtime arg stages (offset+count > 32 => error); the
// `typed_arguments` test mirrors clamp's `arguments` (kInputArgTypes); u32 is the
// only passing input type. `must_use` checks the result is consumed.
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,extractBits",
    "Validation tests for the extractBits() builtin.");
...
CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of extractBits() never errors "
          "on valid inputs")
...
// ---- kInputArgTypes (typesToArguments + manual entries) --------------------
struct InputArg {
    std::string name;
    std::string preamble;
    std::string arg;
...
// typesToArguments([Type.u32], true) and the failing-type set
// ([...kFloatScalarsAndVectors, bool, mat2x2f], false). Each key is type.toString()
// and the arg is type.create(0).wgsl().
...
CTS_TEST(g, "typed_arguments")
    .desc("Test compilation validation of extractBits with variously typed arguments")
    .params([](ParamsBuilder u) {
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            ...
            "\n        _ = extractBits(" + input.arg + "," + offset.arg + "," + count.arg +
            ");\n        return vec4<f32>(.4, .2, .3, .1);\n      }";
        t.expectCompileResult(expectedResult, code);
    });

CTS_TEST(g, "must_use")
    .desc("Result of extractBits must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use,
                              std::string("fn f() { ") + useIt + "extractBits(1u,0u,1u); }");
    });
