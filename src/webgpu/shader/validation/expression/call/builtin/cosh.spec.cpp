...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/cosh.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,cosh", "Validation tests for the cosh() builtin.");
...
CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of cosh() rejects invalid "
          "values")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const bool expectedResult = b::isRepresentable(std::cosh(num), floatResultKind(ty));
        b::validateConstOrOverrideBuiltinEval(t, "cosh", expectedResult,
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of cosh with variously shaped and typed arguments")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(std::string(c.name) == "good",
                              std::string("const c = cosh") + c.suffix + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of cosh must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, "fn f() { " + useIt + "cosh(1.2); }");
    });
