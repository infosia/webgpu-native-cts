...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/log2.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,log2", "Validation tests for the log2() builtin.");

CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of log2() inputs rejects "
          "invalid values")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const bool expectedResult = num > 0;
        b::validateConstOrOverrideBuiltinEval(t, "log2", expectedResult,
...
CTS_TEST(g, "integer_argument")
    .desc("Validates that scalar and vector integer arguments are rejected by log2()")
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of log2 with variously shaped and typed arguments")
    .params([](ParamsBuilder u) { return u.combine("arg", argNames()); })
    .fn([](ShaderValidationTest& t) {
        const std::string arg = t.param<std::string>("arg");
        const std::string code = std::string("const c = log2") + findArgs(arg) + ";";
        t.expectCompileResult(arg == "good", code);
    });

CTS_TEST(g, "must_use")
    .desc("Result of log2 must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, "fn f() { " + useIt + "log2(1.1); }");
    });
