...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/unpack2x16snorm.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
constexpr const char* kBuiltin = "unpack2x16snorm";
constexpr const char* kReturnType = "vec2<f32>";

TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,unpack2x16snorm",
    "Validation tests for the unpack2x16snorm builtin.");
...
// kArgCases: name -> (wgsl call-args spelling, hasF16 element, includesGood).
struct ArgCase {
    const char* name;
    const char* args;  // contents between the parentheses
    bool hasF16;
    bool good;
};
...
CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of unpack2x16snorm rejects "
          "invalid values.")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const bool expectedResult = value >= 0 && value <= static_cast<int64_t>(b::kU32Max);
        b::validateConstOrOverrideBuiltinEval(t, kBuiltin, expectedResult,
                                              {b::createBuiltinValue(ty, rv)}, stage);
    });

CTS_TEST(g, "arguments")
    .desc("Test that unpack2x16snorm is validated correctly when called with different arguments.")
    .params([](ParamsBuilder u) {
...
CTS_TEST(g, "must_use")
    .desc("Result of unpack2x16snorm must be used")
