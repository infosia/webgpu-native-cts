...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/textureSampleGrad.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// Upstream CTS: Copyright (c) 2022 The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) 2026 Kota Iguchi, BSD-3-Clause.
//
// Validation tests for the textureSampleGrad() builtin: coords / array_index /
// ddX / ddY / offset parameter types (ddX/ddY share the coords type), the
// const-expression and [-8,7] range requirements on offset, the return type,
// and incompatible texture types. The kValidTextureSampleGradParameterTypes
// table is ported locally (coordsArgType / hasArrayIndexArg / offsetArgType per
// textureType), preserving the upstream object key order. .specURL is dropped.
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,textureSampleGrad",
    "Validation tests for the textureSampleGrad() builtin.");

// kValidTextureSampleGradParameterTypes (object key order preserved).
// Note: ddX and ddY parameter types match coords so we use coordsArgType for both.
struct SampleArgs {
...
const std::vector<SampleArgs>& kValidTextureSampleGradParameterTypes() {
    using bt::ScalarKind;
...
const SampleArgs* sampleArgsByName(const std::string& name) {
    for (const SampleArgs& s : kValidTextureSampleGradParameterTypes()) {
...
CTS_TEST(g, "return_type")
    .desc("Validates the return type of textureSampleGrad is the expected type.")
...
CTS_TEST(g, "coords_argument")
    .desc("Validates that only incorrect coords arguments are rejected by textureSampleGrad")
    .params([](ParamsBuilder u) {
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            "\n@group(0) @binding(0) var s: sampler;\n@group(0) @binding(1) var t: " + textureType +
            ";\n@fragment fn fs() -> @location(0) vec4f {\n  let v = textureSampleGrad(t, s, " +
            coordWGSL + arrayWGSL + ", " + ddWGSL + ", " + ddWGSL + offsetWGSL +
            ");\n  return vec4f(0);\n}\n";
...
CTS_TEST(g, "array_index_argument")
    .desc("Validates that only incorrect array_index arguments are rejected by textureSampleGrad")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
            .filter([](const ParamRecord& p) {
                const SampleArgs* s =
...
CTS_TEST(g, "ddX_argument")
    .desc("Validates that only incorrect ddX arguments are rejected by textureSampleGrad")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
            .combine("ddxType", bt::typeNames(bt::kAllScalarsAndVectors()))
            .beginSubcases()
            .combine("value", {Value(-1), Value(0), Value(1)})
            .filter([](const ParamRecord& p) {
                return !b::isUnsignedType(
                           bt::typeByName(valueAs<std::string>(*findParam(p, "ddxType")))) ||
                       valueAs<int64_t>(*findParam(p, "value")) >= 0;
            })
            .expand("offset", [](const ParamRecord& p) {
                const SampleArgs* s =
    ...
    .fn([](ShaderValidationTest& t) {
        const std::string textureType = t.param<std::string>("textureType");
        const std::string ddxType = t.param<std::string>("ddxType");
        const int64_t value = t.param<int64_t>("value");
        const bool offset = t.param<bool>("offset");
        const bt::Type ddxArgType = bt::typeByName(ddxType);
        const SampleArgs* s = sampleArgsByName(textureType);

        const bt::Type ddxRequiredType = s->coordsArgType;
        const std::string coordWGSL = bt::createWgsl(s->coordsArgType, 0);
        const std::string arrayWGSL = s->hasArrayIndexArg ? ", 0" : "";
        const std::string ddXWGSL = bt::createWgsl(ddxArgType, static_cast<long long>(value));
        const std::string ddYWGSL = bt::createWgsl(s->coordsArgType, 0);
        const std::string offsetWGSL = offset ? (", " + bt::createWgsl(s->offsetArgType, 0)) : "";

        const std::string code =
            "\n@group(0) @binding(0) var s: sampler;\n@group(0) @binding(1) var t: " + textureType +
            ";\n@fragment fn fs() -> @location(0) vec4f {\n  let v = textureSampleGrad(t, s, " +
            coordWGSL + arrayWGSL + ", " + ddXWGSL + ", " + ddYWGSL + offsetWGSL +
            ");\n  return vec4f(0);\n}\n";
        const bool expectSuccess = bt::isConvertible(ddxArgType, ddxRequiredType);
        t.expectCompileResult(expectSuccess, code);
    });

CTS_TEST(g, "ddY_argument")
    .desc("Validates that only incorrect ddY arguments are rejected by textureSampleGrad")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
            .combine("ddyType", bt::typeNames(bt::kAllScalarsAndVectors()))
            .beginSubcases()
            .combine("value", {Value(-1), Value(0), Value(1)})
            .filter([](const ParamRecord& p) {
                return !b::isUnsignedType(
                           bt::typeByName(valueAs<std::string>(*findParam(p, "ddyType")))) ||
                       valueAs<int64_t>(*findParam(p, "value")) >= 0;
            })
            .expand("offset", [](const ParamRecord& p) {
                const SampleArgs* s =
    ...
    .fn([](ShaderValidationTest& t) {
        const std::string textureType = t.param<std::string>("textureType");
        const std::string ddyType = t.param<std::string>("ddyType");
        const int64_t value = t.param<int64_t>("value");
        const bool offset = t.param<bool>("offset");
        const bt::Type ddyArgType = bt::typeByName(ddyType);
        const SampleArgs* s = sampleArgsByName(textureType);

        const bt::Type ddyRequiredType = s->coordsArgType;
        const std::string coordWGSL = bt::createWgsl(s->coordsArgType, 0);
        const std::string arrayWGSL = s->hasArrayIndexArg ? ", 0" : "";
        const std::string ddXWGSL = bt::createWgsl(s->coordsArgType, 0);
        const std::string ddYWGSL = bt::createWgsl(ddyArgType, static_cast<long long>(value));
        const std::string offsetWGSL = offset ? (", " + bt::createWgsl(s->offsetArgType, 0)) : "";
        ...
        const std::string code =
            ...
            coordWGSL + arrayWGSL + ", " + ddXWGSL + ", " + ddYWGSL + offsetWGSL +
            ");\n  return vec4f(0);\n}\n";
        const bool expectSuccess = bt::isConvertible(ddyArgType, ddyRequiredType);
        t.expectCompileResult(expectSuccess, code);
    });

CTS_TEST(g, "offset_argument")
    .desc("Validates that only incorrect offset arguments are rejected by textureSampleGrad")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
            .filter([](const ParamRecord& p) {
                const SampleArgs* s =
...
CTS_TEST(g, "offset_argument,non_const")
    .desc("Validates that only non-const offset arguments are rejected by textureSampleGrad")
    .params([](ParamsBuilder u) {
        return u.combine("textureType", sampleTextureTypeKeys())
...
CTS_TEST(g, "texture_type")
    .desc("Validates that incompatible texture types don't work with textureSampleGrad")
    .params([](ParamsBuilder u) {
...
CTS_TEST(g, "must_use")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        const std::string code =
            ...
            "sampler;\n    fn foo() {\n      " +
            std::string(use ? "_ =" : "") +
            " textureSampleGrad(t,s,vec2(0,0),vec2(0,0),vec2(0,0));\n    }";
