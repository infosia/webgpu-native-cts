...
// Ported from gpuweb/cts src/webgpu/shader/validation/expression/call/builtin/fma.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// Upstream CTS: Copyright (c) 2022 The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) 2026 Kota Iguchi, BSD-3-Clause.
//
// kValidArgumentTypes = kConvertableToFloatVectors. `values` fills vecN(a),
// vecN(b), vecN(c) and uses ConstantOrOverrideValueChecker on the scalar element
// type to detect OOB (a*b+c) and an ambiguous intermediate (a*b) skip.
...
TestGroup<ShaderValidationTest> g = MakeTestGroup<ShaderValidationTest>(
    "shader,validation,expression,call,builtin,fma", "Validation tests for the fma() builtin.");
...
CTS_TEST(g, "values")
    .desc("Validates that constant evaluation and override evaluation of fma() only errors in cases "
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        vCheck.checkedResult(a * bb + c);
        if (vCheck.allChecksPassed()) {
            vCheck.skipIfCheckFails(a * bb);
        }

        b::validateConstOrOverrideBuiltinEval(
            t, "fma", vCheck.allChecksPassed(),
...
CTS_TEST(g, "args")
    .desc("Test compilation failure of fma with variously shaped and typed arguments")
    .params([](ParamsBuilder u) { return u.combine("arg", argNames()); })
    .fn([](ShaderValidationTest& t) {
        const std::string arg = t.param<std::string>("arg");
        t.expectCompileResult(arg == "good", std::string("const c = fma") + argSuffix(arg) + ";");
    });

CTS_TEST(g, "must_use")
    .desc("Result of fma must be used")
    ...
    .fn([](ShaderValidationTest& t) {
        ...
        t.expectCompileResult(use, std::string("fn f() { ") + useIt + "fma" + argSuffix("good") +
                                       "; }");
    });
