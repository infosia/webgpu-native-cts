// Ported from gpuweb/cts src/webgpu/shader/execution/expression/unary/ai_arithmetic.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution Tests for the abstract integer arithmetic unary expression operations (negation).
...
TestGroup<AllFeaturesMaxLimitsGpuTest> testGroup = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,unary,ai_arithmetic",
...
CTS_TEST(testGroup, "negation").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    for (int64_t e : fullI64Range()) {
        const int64_t neg = static_cast<int64_t>(0ull - static_cast<uint64_t>(e));
        cases.push_back({{CaseValue(abstractInt64(e))}, CaseValue(abstractInt64(neg))});
    }
