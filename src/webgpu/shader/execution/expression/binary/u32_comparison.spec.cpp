// Ported from gpuweb/cts src/webgpu/shader/execution/expression/binary/u32_comparison.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<AllFeaturesMaxLimitsGpuTest> testGroup = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,binary,u32_comparison",
    "Execution Tests for the u32 comparison expressions");
...
using Cmp = std::function<bool(uint32_t, uint32_t)>;

void runCmp(AllFeaturesMaxLimitsGpuTest& t, const std::string& op, const Cmp& cmp) {
    std::vector<Case> cases;
    for (const std::vector<uint32_t>& v : vectorU32Range(2)) {
        cases.push_back({{CaseValue(u32(v[0])), CaseValue(u32(v[1]))},
                         CaseValue(boolean(cmp(v[0], v[1])))});
    }
    run(t, binaryOp(op), {U32, U32}, BOOL, cfgInputSource(t), cfgVectorize(t), cases);
}

CTS_TEST(testGroup, "equals").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    runCmp(t, "==", [](uint32_t a, uint32_t b) { return a == b; });
});
