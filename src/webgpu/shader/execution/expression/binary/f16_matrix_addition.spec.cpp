// Ported from gpuweb/cts src/webgpu/shader/execution/expression/binary/f16_matrix_addition.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution tests for matrix f16 addition (x + y, componentwise). Accuracy: correctly rounded.
// Mirrors f32_matrix_addition but uses the f16 traits + f16 ranges; gated on the 'shader-f16' feature.
...
TestGroup<AllFeaturesMaxLimitsGpuTest> g = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,binary,f16_matrix_addition",
...
fp::MatrixPairToMatrix addMatOp() {
    return [](const std::vector<std::vector<double>>& x,
        ...
        return fp::additionMatrixMatrixInterval(fp::FPKind::F16, x, y);
    };
}
