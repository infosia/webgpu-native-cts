// Ported from gpuweb/cts src/webgpu/shader/execution/expression/binary/f16_addition.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution tests for non-matrix f16 addition (x + y). Accuracy: correctly rounded.
// Mirrors f32_addition but uses the f16 traits + f16 ranges; gated on the 'shader-f16' feature.
...
TestGroup<AllFeaturesMaxLimitsGpuTest> g = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,binary,f16_addition",
...
// FP.f16.additionInterval(x, y).
fp::FPInterval addOp(double x, double y) { return fp::additionInterval(fp::FPKind::F16, x, y); }
...
CTS_TEST(g, "vector").params(sourceVectorize).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    requireF16(t);
    // Uses vectorize to generate vector cases based on the scalar cases.
    auto cases = scalarCases(!isConst(t));
