// Ported from gpuweb/cts src/webgpu/shader/execution/expression/binary/af_subtraction.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution tests for non-matrix abstract-float subtraction (x - y). Accuracy: correctly rounded.
// The abstract-float acceptance intervals are computed natively at f64 precision via
// fp::subtractionInterval(FPKind::Abstract, ...), mirroring the upstream golden lookup tables.
...
TestGroup<AllFeaturesMaxLimitsGpuTest> g = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,binary,af_subtraction",
...
// FP.abstract.subtractionInterval(x, y), computed natively at f64 precision.
fp::FPInterval subOp(double x, double y) { return fp::subtractionInterval(fp::FPKind::Abstract, x, y); }
...
std::vector<Case> scalarCases() {
    return fp::generateScalarPairToIntervalCases(fp::FPKind::Abstract, fp::sparseScalarF64Range(),
                                                 fp::sparseScalarF64Range(), /*finite=*/true, subOp);
}
