// Ported from gpuweb/cts src/webgpu/shader/execution/expression/binary/f16_remainder.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution tests for non-matrix f16 remainder (x % y). Accuracy: derived (x - y * trunc(x / y)).
// Mirrors f32_remainder but uses the f16 traits + f16 ranges; gated on the 'shader-f16' feature.
...
TestGroup<AllFeaturesMaxLimitsGpuTest> g = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,binary,f16_remainder",
...
// FP.f16.remainderInterval(x, y).
fp::FPInterval remOp(double x, double y) { return fp::remainderInterval(fp::FPKind::F16, x, y); }
