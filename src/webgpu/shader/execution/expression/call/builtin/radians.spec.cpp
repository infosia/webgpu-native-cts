// Ported from gpuweb/cts src/webgpu/shader/execution/expression/call/builtin/radians.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution tests for the 'radians' builtin function. radians(e) ~= e * pi/180 (multiplication by
// the constant, correctly-rounded). radians has inherited accuracy, so abstract is only as accurate
// as f32. f32 always unfiltered; abstract finite. f16 is deferred (no Metal oracle).
...
TestGroup<AllFeaturesMaxLimitsGpuTest> g = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,call,builtin,radians",
...
CTS_TEST(g, "abstract_float").params(constOnlyVectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    auto cases = fp::generateScalarToIntervalCases(
        ...
        [](double n) { return fp::radiansInterval(fp::FPKind::F32, n); });
    run(t, builtin("radians"), {scalarType(ScalarKind::AbstractFloat)},
...
CTS_TEST(g, "f32").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    auto cases = fp::generateScalarToIntervalCases(
        ...
        [](double n) { return fp::radiansInterval(fp::FPKind::F32, n); });
    run(t, builtin("radians"), {scalarType(ScalarKind::F32)}, scalarType(ScalarKind::F32),
