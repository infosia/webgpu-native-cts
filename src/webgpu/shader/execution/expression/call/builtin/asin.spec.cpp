// Ported from gpuweb/cts src/webgpu/shader/execution/expression/call/builtin/asin.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution tests for the 'asin' builtin function. asin = span(atan2(n, sqrt(1-n*n)), abs-error)
// over [-1, 1] (inherited/ulp accuracy); abstract uses the f32 asinInterval. f16 is deferred.
...
TestGroup<AllFeaturesMaxLimitsGpuTest> g = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,call,builtin,asin",
...
std::vector<double> asinRange(fp::FPKind kind) {
...
CTS_TEST(g, "abstract_float").params(constOnlyVectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    auto cases = fp::generateScalarToIntervalCases(
        fp::FPKind::Abstract, asinRange(fp::FPKind::Abstract), /*finite=*/true,
        [](double n) { return fp::asinInterval(fp::FPKind::F32, n); });
    run(t, builtin("asin"), {scalarType(ScalarKind::AbstractFloat)},
...
CTS_TEST(g, "f32").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    auto cases = fp::generateScalarToIntervalCases(
        fp::FPKind::F32, asinRange(fp::FPKind::F32), /*finite=*/isConst,
        [](double n) { return fp::asinInterval(fp::FPKind::F32, n); });
    run(t, builtin("asin"), {scalarType(ScalarKind::F32)}, scalarType(ScalarKind::F32),
...
CTS_TEST(g, "f16").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    const bool isConst = cfgInputSource(t) == InputSource::Const;
    auto cases = fp::generateScalarToIntervalCases(
        fp::FPKind::F16, asinRange(fp::FPKind::F16), /*finite=*/isConst,
        [](double n) { return fp::asinInterval(fp::FPKind::F16, n); });
    run(t, builtin("asin"), {scalarType(ScalarKind::F16)}, scalarType(ScalarKind::F16),
        cfgInputSource(t), cfgVectorize(t), cases);
});
