// Ported from gpuweb/cts src/webgpu/shader/execution/expression/call/builtin/trunc.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution tests for the 'trunc' builtin function (correctly-rounded; f32 + abstract-float).
...
TestGroup<AllFeaturesMaxLimitsGpuTest> g = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,call,builtin,trunc",
...
CTS_TEST(g, "abstract_float").params(constOnlyVectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    auto cases = fp::generateScalarToIntervalCases(
        ...
        [](double n) { return fp::truncInterval(fp::FPKind::Abstract, n); });
    run(t, builtin("trunc"), {scalarType(ScalarKind::AbstractFloat)},
...
CTS_TEST(g, "f32").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    auto cases = fp::generateScalarToIntervalCases(
        ...
        [](double n) { return fp::truncInterval(fp::FPKind::F32, n); });
    run(t, builtin("trunc"), {scalarType(ScalarKind::F32)}, scalarType(ScalarKind::F32),
...
CTS_TEST(g, "f16").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    auto cases = fp::generateScalarToIntervalCases(
        ...
        [](double n) { return fp::truncInterval(fp::FPKind::F16, n); });
    run(t, builtin("trunc"), {scalarType(ScalarKind::F16)}, scalarType(ScalarKind::F16),
        cfgInputSource(t), cfgVectorize(t), cases);
});
