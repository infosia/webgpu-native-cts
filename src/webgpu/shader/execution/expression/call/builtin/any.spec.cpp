// Ported from gpuweb/cts src/webgpu/shader/execution/expression/call/builtin/any.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
...
TestGroup<AllFeaturesMaxLimitsGpuTest> testGroup = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,call,builtin,any",
    "Execution tests for the 'any' builtin function.");
...
CTS_TEST(testGroup, "bool")
    ...
    .fn([](AllFeaturesMaxLimitsGpuTest& t) {
        ...
        for (int bits = 0; bits < n; ++bits) {
            ...
            cases.push_back({{inputValue}, CaseValue(boolean(anyTrue))});
        }

        const ExprType paramTy = width == 1 ? scalarType(ScalarKind::Bool)
                                            : vecType(width, ScalarKind::Bool);
        run(t, builtin("any"), {paramTy}, scalarType(ScalarKind::Bool), src, 0, cases);
    });
