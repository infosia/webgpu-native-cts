// Ported from gpuweb/cts src/webgpu/shader/execution/expression/call/builtin/min.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution tests for the 'min' builtin function. Integer variants are bit-exact min; float variants
// use the correctly-rounded minInterval. f32 and abstract use their own trait. f16 is deferred.
...
TestGroup<AllFeaturesMaxLimitsGpuTest> g = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,call,builtin,min",
...
std::vector<Case> floatCases(fp::FPKind kind) {
    ...
    return fp::generateScalarPairToIntervalCases(
        ...
        [kind](double x, double y) { return fp::minInterval(kind, x, y); });
}
    ...
    for (int64_t e : vals) {
        for (int64_t f : vals) {
            cases.push_back(Case({CaseValue(abstractInt64(e)), CaseValue(abstractInt64(f))},
                                 CaseValue(abstractInt64(std::min(e, f)))));
        }
    }
    run(t, builtin("min"), {scalarType(ScalarKind::AbstractInt), scalarType(ScalarKind::AbstractInt)},
...
CTS_TEST(g, "u32").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    for (uint32_t e : vals) {
        for (uint32_t f : vals) {
            cases.push_back(Case({CaseValue(u32(e)), CaseValue(u32(f))}, CaseValue(u32(std::min(e, f)))));
        }
    }
    run(t, builtin("min"), {scalarType(ScalarKind::U32), scalarType(ScalarKind::U32)},
...
CTS_TEST(g, "i32").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    for (int32_t e : vals) {
        for (int32_t f : vals) {
            cases.push_back(Case({CaseValue(i32(e)), CaseValue(i32(f))}, CaseValue(i32(std::min(e, f)))));
        }
    }
    run(t, builtin("min"), {scalarType(ScalarKind::I32), scalarType(ScalarKind::I32)},
...
CTS_TEST(g, "abstract_float").params(constOnlyVectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    run(t, builtin("min"), {scalarType(ScalarKind::AbstractFloat), scalarType(ScalarKind::AbstractFloat)},
...
CTS_TEST(g, "f32").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    run(t, builtin("min"), {scalarType(ScalarKind::F32), scalarType(ScalarKind::F32)},
...
CTS_TEST(g, "f16").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    auto cases = floatCases(fp::FPKind::F16);
    run(t, builtin("min"), {scalarType(ScalarKind::F16), scalarType(ScalarKind::F16)},
