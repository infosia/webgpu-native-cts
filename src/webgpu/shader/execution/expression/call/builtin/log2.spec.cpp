// Ported from gpuweb/cts src/webgpu/shader/execution/expression/call/builtin/log2.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution tests for the 'log2' builtin function. log2 has abs-error 2^-21 on [0.5,2], else
// ULP(3) for f32, over the >0 domain; abstract uses the f32 log2Interval. f16 is deferred.
...
TestGroup<AllFeaturesMaxLimitsGpuTest> g = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,call,builtin,log2",
...
std::vector<double> log2Range(fp::FPKind kind) {
...
CTS_TEST(g, "abstract_float").params(constOnlyVectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    auto cases = fp::generateScalarToIntervalCases(
        fp::FPKind::Abstract, log2Range(fp::FPKind::Abstract), /*finite=*/true,
        [](double n) { return fp::log2Interval(fp::FPKind::F32, n); });
    run(t, builtin("log2"), {scalarType(ScalarKind::AbstractFloat)},
...
CTS_TEST(g, "f32").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    auto cases = fp::generateScalarToIntervalCases(
        fp::FPKind::F32, log2Range(fp::FPKind::F32), /*finite=*/isConst,
        [](double n) { return fp::log2Interval(fp::FPKind::F32, n); });
    run(t, builtin("log2"), {scalarType(ScalarKind::F32)}, scalarType(ScalarKind::F32),
...
CTS_TEST(g, "f16").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    const bool isConst = cfgInputSource(t) == InputSource::Const;
    auto cases = fp::generateScalarToIntervalCases(
        fp::FPKind::F16, log2Range(fp::FPKind::F16), /*finite=*/isConst,
        [](double n) { return fp::log2Interval(fp::FPKind::F16, n); });
    run(t, builtin("log2"), {scalarType(ScalarKind::F16)}, scalarType(ScalarKind::F16),
        cfgInputSource(t), cfgVectorize(t), cases);
});
