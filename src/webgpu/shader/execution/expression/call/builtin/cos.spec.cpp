// Ported from gpuweb/cts src/webgpu/shader/execution/expression/call/builtin/cos.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution tests for the 'cos' builtin function. cos has absolute accuracy (2^-11 for f32) over
...
TestGroup<AllFeaturesMaxLimitsGpuTest> g = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,call,builtin,cos",
...
// linearRange(-pi, pi, 100) ++ scalarRange().
std::vector<double> cosRange(fp::FPKind kind) {
...
CTS_TEST(g, "abstract_float").params(constOnlyVectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    // abstract: FP.abstract generator (f64 range), finite filter, f32 cosInterval op (inherited).
    auto cases = fp::generateScalarToIntervalCases(
        fp::FPKind::Abstract, cosRange(fp::FPKind::Abstract), /*finite=*/true,
        [](double n) { return fp::cosInterval(fp::FPKind::F32, n); });
    run(t, builtin("cos"), {scalarType(ScalarKind::AbstractFloat)},
...
CTS_TEST(g, "f32").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    auto cases = fp::generateScalarToIntervalCases(
        fp::FPKind::F32, cosRange(fp::FPKind::F32), /*finite=*/false,
        [](double n) { return fp::cosInterval(fp::FPKind::F32, n); });
    run(t, builtin("cos"), {scalarType(ScalarKind::F32)}, scalarType(ScalarKind::F32),
...
CTS_TEST(g, "f16").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    ...
    auto cases = fp::generateScalarToIntervalCases(
        fp::FPKind::F16, cosRange(fp::FPKind::F16), /*finite=*/false,
        [](double n) { return fp::cosInterval(fp::FPKind::F16, n); });
    run(t, builtin("cos"), {scalarType(ScalarKind::F16)}, scalarType(ScalarKind::F16),
        cfgInputSource(t), cfgVectorize(t), cases);
});
