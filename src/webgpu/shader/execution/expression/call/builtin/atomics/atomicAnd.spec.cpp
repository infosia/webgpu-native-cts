// Ported from gpuweb/cts src/webgpu/shader/execution/expression/call/builtin/atomics/atomicAnd.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85
...
TestGroup<AllFeaturesMaxLimitsGpuTest> testGroup = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,call,builtin,atomics,atomicAnd",
    "Atomically read, and and store value.");

void runAnd(AllFeaturesMaxLimitsGpuTest& t, bool workgroup) {
    ...
    const uint32_t initValue = 0xffffffffu;
    ...
    const std::string op =
        "let i = map_id(u32(id));\n"
        "  atomicAnd(&" + target + "[i / 32], ~(" + type + "(1) << i))";
...
CTS_TEST(testGroup, "and_storage")
    .params([](ParamsBuilder u) { return mapIdParams(u); })
    .fn([](AllFeaturesMaxLimitsGpuTest& t) { runAnd(t, false); });

CTS_TEST(testGroup, "and_workgroup")
    .params([](ParamsBuilder u) { return mapIdParams(u); })
    .fn([](AllFeaturesMaxLimitsGpuTest& t) { runAnd(t, true); });
