// Ported from gpuweb/cts src/webgpu/shader/execution/expression/call/builtin/atomics/atomicOr.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85
...
TestGroup<AllFeaturesMaxLimitsGpuTest> testGroup = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,call,builtin,atomics,atomicOr",
    "Atomically read, or and store value.");

void runOr(AllFeaturesMaxLimitsGpuTest& t, bool workgroup) {
    ...
    const std::string op =
        "let i = map_id(u32(id));\n"
        "  atomicOr(&" + target + "[i / 32], " + type + "(1) << i)";

    std::vector<uint32_t> expected(static_cast<size_t>(numElements) * (workgroup ? dispatchSize : 1), 0);
    ...
    if (workgroup) {
        runWorkgroupVariableTest(t, workgroupSize, dispatchSize, numElements, 0, op, expected, scalarType, extra);
    } else {
        runStorageVariableTest(t, workgroupSize, dispatchSize, numElements, 0, op, expected, scalarType, extra);
    }
}

CTS_TEST(testGroup, "or_storage")
    .params([](ParamsBuilder u) { return mapIdParams(u); })
    .fn([](AllFeaturesMaxLimitsGpuTest& t) { runOr(t, false); });

CTS_TEST(testGroup, "or_workgroup")
    .params([](ParamsBuilder u) { return mapIdParams(u); })
    .fn([](AllFeaturesMaxLimitsGpuTest& t) { runOr(t, true); });
