// Ported from gpuweb/cts src/webgpu/shader/execution/expression/call/builtin/subgroupAny.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85
...
TestGroup<sg::SubgroupTest> g = MakeTestGroup<sg::SubgroupTest>(
    "shader,execution,expression,call,builtin,subgroupAny",
    "Execution tests for subgroupAny");
...
std::optional<std::string> checkAny(
    ...
    for (uint32_t inv = 0; inv < numInvs; ++inv) {
        ...
        uint32_t v = it == expected.end() ? 0u : it->second;
        v |= input[inv];
    ...
    .fn([](sg::SubgroupTest& t) {
        ...
        sg::runComputeTest(
            ...
            [wgThreads, inputData](const std::vector<uint32_t>& metadata,
                ...
                return checkAny(metadata, output, wgThreads, inputData,
    ...
    .fn([](sg::SubgroupTest& t) {
        ...
        sg::runComputeTest(
            ...
            [wgThreads, inputData, filter](
                const std::vector<uint32_t>& metadata,
                const std::vector<uint32_t>& output) -> std::optional<std::string> {
                return checkAny(metadata, output, wgThreads, inputData, filter);
            });
    });

// Checks subgroupAny results from a fragment shader.
std::optional<std::string> checkFragmentAny(
    ...
    .fn([](sg::SubgroupTest& t) {
        ...
        sg::runFragmentTest(
            ...
            [inputData, size](
                ...
                return checkFragmentAny(data, inputData, WGPUTextureFormat_RG32Uint, size[0],
                                        size[1]);
            });
    });
