// Ported from gpuweb/cts src/webgpu/shader/execution/expression/call/builtin/tanh.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85 (see docs/UPSTREAM.md).
// SPDX-License-Identifier: BSD-3-Clause
// Upstream CTS: Copyright (c) The WebGPU CTS Contributors, BSD-3-Clause.
// Port:         Copyright (c) the webgpu-native-cts contributors, BSD-3-Clause.
//
// Execution tests for the 'tanh' builtin function. tanh = span(sinh/cosh, abs-error 1e-5)
// (inherited accuracy); abstract uses the f32 tanhInterval. f16 is deferred (no Metal oracle).
...
TestGroup<AllFeaturesMaxLimitsGpuTest> g = MakeTestGroup<AllFeaturesMaxLimitsGpuTest>(
    "shader,execution,expression,call,builtin,tanh",
...
CTS_TEST(g, "abstract_float").params(constOnlyVectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    auto cases = fp::generateScalarToIntervalCases(
        ...
        [](double n) { return fp::tanhInterval(fp::FPKind::F32, n); });
    run(t, builtin("tanh"), {scalarType(ScalarKind::AbstractFloat)},
...
CTS_TEST(g, "f32").params(vectorizeParams).fn([](AllFeaturesMaxLimitsGpuTest& t) {
    auto cases = fp::generateScalarToIntervalCases(
        ...
        [](double n) { return fp::tanhInterval(fp::FPKind::F32, n); });
    run(t, builtin("tanh"), {scalarType(ScalarKind::F32)}, scalarType(ScalarKind::F32),
