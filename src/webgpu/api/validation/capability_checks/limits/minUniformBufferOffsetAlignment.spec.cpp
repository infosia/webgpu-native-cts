// Ported from gpuweb/cts src/webgpu/api/validation/capability_checks/limits/minUniformBufferOffsetAlignment.spec.ts @ b507bd117e53db86f2fb52d0d858d3ae7d684a85
#include <algorithm>
#include <cstdint>
#include <string>
...
class MinUniformBufferOffsetAlignmentTest : public LimitTest {
  public:
    const char* limitName() const override { return "minUniformBufferOffsetAlignment"; }
};

TestGroup<MinUniformBufferOffsetAlignmentTest> testGroup = MakeTestGroup<MinUniformBufferOffsetAlignmentTest>(
    "api,validation,capability_checks,limits,minUniformBufferOffsetAlignment",
    "API Validation Tests for minUniformBufferOffsetAlignment.");
...
uint64_t getDeviceLimitToRequest(std::string_view limitTest, uint64_t defaultLimit, uint64_t minimumLimit) {
    ...
    if (limitTest == "betweenDefaultAndMinimum") {
        return std::min(minimumLimit, powerOfTwo((defaultLog + minimumLog) / 2));
    }
...
void runCreateBindGroupTest(MinUniformBufferOffsetAlignmentTest& t) {
...
void runSetBindGroupTest(MinUniformBufferOffsetAlignmentTest& t) {
    const uint64_t requestedLimit =
        getDeviceLimitToRequest(t.param<std::string>("limitTest"), t.defaultLimit, t.adapterLimit);
    const uint64_t testValue = getTestValue(t.param<std::string>("testValueName"), requestedLimit);
    t.testDeviceWithSpecificLimits(requestedLimit, testValue, [&](const SpecificLimitTestInputs& inputs) {
...
CTS_TEST(testGroup, "createBindGroup,at_over")
    .desc("Test using createBindGroup at and over minUniformBufferOffsetAlignment limit")
    .params([](ParamsBuilder u) { return kMinimumLimitBaseParams(u); })
    .fn([](MinUniformBufferOffsetAlignmentTest& t) { runCreateBindGroupTest(t); });

CTS_TEST(testGroup, "setBindGroup,at_over")
    .desc("Test using setBindGroup at and over minUniformBufferOffsetAlignment limit")
    .params([](ParamsBuilder u) { return kMinimumLimitBaseParams(u); })
    .fn([](MinUniformBufferOffsetAlignmentTest& t) { runSetBindGroupTest(t); });

CTS_TEST(testGroup, "validate,powerOf2")
    .desc("Verify that minUniformBufferOffsetAlignment is power of 2")
    .fn([](MinUniformBufferOffsetAlignmentTest& t) {
...
CTS_TEST(testGroup, "validate,greaterThanOrEqualTo32")
    .desc("Verify that minUniformBufferOffsetAlignment is >= 32")
    .fn([](MinUniformBufferOffsetAlignmentTest& t) {
